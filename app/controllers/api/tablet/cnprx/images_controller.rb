class Api::Tablet::Cnprx::ImagesController < ApplicationController
  ...
  def image_copy_to_ftp(custserial: nil, ch_cd: nil, measureno: nil, number: nil, type: nil)
    ...
    folder_create_command << "' -u 'janus:pielgahn2012!' -Q '-MKD "
    folder_create_command << make_dir_command
    folder_create_command << "' --ftp-create-dirs"
    system(folder_create_command)

    # Curl -p - --insecure"ftp://165.244.88.27/CNP/" --user"janus:pielgahn2012!" -Q "-MKD /CNP/test"--ftp-create-dirs
    # 100-P 만들기 end
    # Success!!!!
    ...
    # "curl -p --insecure 'ftp://165.244.88.27/CNP/900-P/' -u 'janus:pielgahn2012!' -Q '-MKD /CNP/900-P/839-1' --ftp-create-dirs"
    # 100-P/839-1 만들기
    # Success!!!!
    ...
    file_copy_command << "' -u 'janus:pielgahn2012!' -T '/home/janustabuser/lgcare/current/public"
    file_copy_command << file_path
    file_copy_command << "' --ftp-create-dirs"
    (0..10).each do |i|
      break if system(file_copy_command)
    end
    # "curl -p --insecure 'ftp://165.244.88.27/CNP/900-P/839-1/' -u 'janus:pielgahn2012!' -T '/home/janustabuser/lgcare/current/public/CNP/900-P/839-1/839-1_Sym_L_1.jpg' --ftp-create-dirs"
    #  ls_command = "curl -l 'ftp://165.244.88.27/CNP/900-P/839-1/' -u 'janus:pielgahn2012!' --ftp-create-dirs"
  end
end
