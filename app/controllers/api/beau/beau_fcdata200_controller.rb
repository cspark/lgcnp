class Api::Beau::BeauFcdata200Controller < Api::ApplicationController
  def show
    list = Fcdata200.where(custserial: params[:id], measureno: params[:measureno], ch_cd: params[:ch_cd]).order(measureno: :desc)
    if list.count > 0
  ...
  def create
    ...
    if !Fcdata200.where(custserial: params[:custserial], ch_cd: params[:ch_cd], measureno: params[:measureno]).first.nil?
      render :text => "Data exist!!!", status: 204
      return
    end
    fcdata = Fcdata200.new(permitted_params)
    t = Time.now
