class Api::Cnp::CnpUserController < Api::ApplicationController
  def index
    ...
    if params.has_key?(:phone)
      if params[:ch_cd] == "CNP" || params[:ch_cd] == "CLAB"
        # user = Custinfo.where(n_cust_id: nil).where(ch_cd: params[:ch_cd]).where(custname: params[:custname], birthyy: params[:birthyy], birthmm: birthmm, birthdd: birthdd, phone: params[:phone]).order("UPTDATE desc").first
        user = Custinfo.where(custname: params[:custname], birthyy: params[:birthyy], birthmm: birthmm, birthdd: birthdd, phone: params[:phone]).order("UPTDATE desc").first
      else
        # user = Custinfo.where.not(n_cust_id: nil).where(custname: params[:custname], birthyy: params[:birthyy], birthmm: birthmm, birthdd: birthdd, phone: params[:phone]).order("UPTDATE desc").first
        user = Custinfo.where(custname: params[:custname], birthyy: params[:birthyy], birthmm: birthmm, birthdd: birthdd, phone: params[:phone]).order("UPTDATE desc").first
      end
    else
      if params[:ch_cd] == "CNP" || params[:ch_cd] == "CLAB"
        ...
        user = Custinfo.where(custname: params[:custname], birthyy: params[:birthyy], birthmm: birthmm, birthdd: birthdd).order("UPTDATE desc").first
      else
  ...
  def permitted_params
    params.permit(:custserial, :ch_cd, :n_cust_id, :custname, :sex, :age, :birthyy, :birthmm, :birthdd, :phone, :uptdate, :shop_cd, :is_agree_privacy, :is_agree_after, :is_agree_marketing, :is_agree_thirdparty_info, :address, :lastanaldate, :measureno, :email, :age)
  end
end
