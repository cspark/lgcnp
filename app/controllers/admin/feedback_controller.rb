class Admin::FeedbackController < Admin::AdminApplicationController
  ...
  def index
    ...
    if Rails.env.production? || Rails.env.staging?
      ...
      @tablet_interviews_3_months_ago = Fctabletinterview.where(ch_cd: "CNP").where(custserial: serial_array2).where(fcdata_id: measureno_array).where("to_char(to_date(uptdate), 'YYYY-MM-DD') LIKE ?", ((@date - 3.months).to_s)).order("uptdate desc")
