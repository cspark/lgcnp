class Admin::DataController < Admin::AdminApplicationController
  ...
  def list
    ...
    if @select_skin_anxiety1.blank?
      ...
      scoped = scoped.where(worry_skin_1: @select_skin_anxiety1)
    end

    if @select_skin_anxiety2.blank?
      scoped = scoped.where(worry_skin_2: ["!!"])
    else
      scoped = scoped.where(worry_skin_2: @select_skin_anxiety2)
    end
