class FcgeneInterview < ApplicationRecord
  ...
  has_many :fcdatas, ->(gene_interview){ unscope(where: :fcgene_interview_id).where("custserial = ? AND measureno = ?", gene_interview.custserial, gene_interview.measureno) } if Rails.env.production?  || Rails.env.staging?
